...
impl PirEngine for StubPirEngine {
    ...
    fn answer_query(
        ...
        let start = idx as usize * DECRYPT_ROW_BYTES;
        let end = start + DECRYPT_ROW_BYTES;
        if end > state.db.len() {
            return Err(StubPirError::OutOfRange(idx, state.num_items));
        }
...
mod tests {
    use super::*;
    use decryption_types::{DECRYPT_DB_BYTES, DECRYPT_DB_ROWS};
    ...
    fn stub_invalid_query() {
        ...
        let db = vec![0u8; DECRYPT_DB_BYTES];
        let state = engine.setup(&db, &scenario).unwrap();
    ...
    fn stub_out_of_range() {
        ...
        let db = vec![0u8; DECRYPT_DB_BYTES];
        let state = engine.setup(&db, &scenario).unwrap();
        let idx = (DECRYPT_DB_ROWS as u32).to_le_bytes();
